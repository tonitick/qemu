...
struct NestedStruct {
    ...
    union {
        ...
        size_t offset;      // if not pointer, the offset within parent struct
    } loc;
...
static inline void ns_init_ptr(struct NestedStruct *n, unsigned int addr, size_t size) {
    n->is_pointer  = true;
    n->size        = size;
    n->loc.addr    = addr;
    n->fields      = NULL;
...
static inline struct NestedStruct *ns_new_ptr(unsigned int addr, size_t size) {
    struct NestedStruct *n = (struct NestedStruct *)calloc(1, sizeof *n);
    if (n) ns_init_ptr(n, addr, size);
    return n;
}
