...
// path log recording
// ---------------------------------------------------------------
// utility: check if any path log is ready for dumping (i.e., has log count >= MAX_PER_PATH_LOG_SIZE), 
//          if yes, dump the related path logs to files under dump_dir, and return 1; otherwise return 0
void record_trace_values(const uint64_t *seq, size_t len, const ArgSetting *arg_settings, size_t arg_count, const RetSetting *ret_settings, size_t ret_count);
                                ...
                                // const ValueUnion *ret_vals)
{
    ...
    //     }
    // }
    append_log_from_arg_settings(e->args, arg_settings, arg_count);
    ...
    //     }
    // }
    append_log_from_ret_settings(e->rets, ret_settings, ret_count);
    ...
    //     }
    // }
    append_concrete_inputs_to_path_log(e, arg_settings, arg_count);
}

// return 1 on success, 0 on failure (e.g., not all path logs are ready for dump)
...
int check_path_log_size_and_dump(char* dump_dir) {
    ...
    if (is_path_log_ready_for_dump(arg_count)) {
        ...
        HASH_ITER(hh, g_map, e, tmp) {
            ...
            char iotxt_filepath[256] = {0};
            // snprintf(filepath, sizeof(filepath), "%s/path_id_%d_len_%zu.txt", dump_dir, path_id++, e->len);
            snprintf(iotxt_filepath, sizeof(iotxt_filepath), "%s/in_outs.txt", path_dir);
            ...
            // }
            // fclose(f);
            if (dump_io_pairs_from_path_log_entry_to_txt(e, iotxt_filepath, arg_settings, arg_count, ret_settings, ret_count) != 0) {
                fprintf(stderr, "Failed to dump io pairs from path log entry to txt for path_id %d\n", path_id-1);
        ...
        // cJSON_Delete(ci_output_root);
        if (dump_ret_settings_to_call_interface_json_file(calling_interface_output_filepath, ret_settings, ret_count) != 0) {
            fprintf(stderr, "Failed to dump calling interface output to json file\n");
            exit(EXIT_FAILURE);
        }
...
void record_sub_semantic_trace_values(void) {
    ...
    //     }
    // }
    append_log_from_arg_settings(arglog_subsem, arg_settings, arg_count);
    // for (size_t i = 0; i < ret_count; ++i) {
    ...
    //     }
    // }
    append_log_from_ret_settings(retlog_subsem, ret_settings, ret_count);
}

// utility: check if sub-semantic log is ready for dumping (i.e., all log count >= MAX_PER_PATH_LOG_SIZE), 
//          if yes, dump the related logs to dump_dir and return 1; otherwise return 0
...
int check_sub_semantic_log_size_and_dump(char* dump_dir) {
    ...
    if (is_sub_semantic_log_ready_for_dump(arg_count)) {
        char iotxt_filepath[256] = {0};
        snprintf(iotxt_filepath, sizeof(iotxt_filepath), "%s/in_outs.txt", dump_dir);
        // FILE *f = fopen(filepath, "w");
        // if (!f) {
