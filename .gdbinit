...
run --plugin ./tests/tcg/plugins/libvirtual.so,logger=ws/log_config.txt,virtual=ws/virtuals.txt,detour=ws/detours.txt,modifier=ws/modifiers.txt,args=ws/rand_setting.json,outs=ws/out_setting.json -d in_asm,op -D qemu.log -machine pixhawk1 -monitor telnet:127.0.0.1:5555,server,nowait -semihosting --semihosting-config enable=on,target=native -qmp unix:/tmp/qmp-sock,server,nowait -kernel ../ws/m4_debug -serial stdio -nographic -gdb tcp::1235 -S
